...
const recurrentUnwrapGroupingColumnModel = (
  ...
  if (isLeaf(columnGroupNode)) {
    if (unwrappedGroupingModelToComplet[columnGroupNode.field] !== undefined) {
      throw new Error(
        [
          `columnGroupingModel contains duplicated field`,
...
const createGroupLookup = (columnGroupingModel: GridColumnNode[]): GridColumnGroupLookup => {
  ...
  columnGroupingModel.forEach((node) => {
    ...
    if (!groupId) {
      throw new Error(
        'An element of the columnGroupingModel does not have either `field` or `groupId`.',
      );
    }
    if (!children) {
      console.warn(`group groupId=${groupId} has no children.`);
    }
    const groupParam = { ...other, groupId };
    const subTreeLookup = createGroupLookup(children);
    if (subTreeLookup[groupId] !== undefined || groupLookup[groupId] !== undefined) {
      throw new Error(
        `The groupId ${groupId} is used multiple times in the columnGroupingModel.`,
      );
    }
