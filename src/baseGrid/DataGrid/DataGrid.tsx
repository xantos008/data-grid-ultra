...
DataGridRaw.propTypes = {
  ...
  columns: chainPropTypes(PropTypes.array.isRequired, (props) => {
    ...
    if (props.columns && props.columns.some((column) => column.resizable)) {
      return new Error(
        [
          `\`column.resizable = true\` is not a valid prop.`,
  ...
  pageSize: chainPropTypes(PropTypes.number, (props: any) => {
    if (props.pageSize && props.pageSize > MAX_PAGE_SIZE) {
      return new Error(
        [
          `\`<DataGrid pageSize={${props.pageSize}} />\` is not a valid prop.`,
  ...
  pagination: (props: any) => {
    if (props.pagination === false) {
      return new Error(
        [
          '`<DataGrid pagination={false} />` is not a valid prop.',
          'Infinite scrolling is not available in the MIT version.',
          '',
          'You need to upgrade to DataGridPro or DataGridUltra component to disable the pagination.',
        ].join('\n'),
      );
    }
    return null;
  ...
  selectionModel: chainPropTypes(
    ...
    (props: any) => {
      if (
        ...
        return new Error(
          [
            `\`<DataGrid selectionModel={${JSON.stringify(
              props.selectionModel,
            )}} />\` is not a valid prop.`,
            'selectionModel can only be of 1 item in DataGrid.',
            '',
            'You need to upgrade to DataGridPro or DataGridUltra component to unlock multiple selection.',
          ].join('\n'),
        );
      }
      return null;
