...
DataGridUltraRaw.propTypes = {
  ...
  checkboxSelectionVisibleOnly: chainPropTypes(PropTypes.bool, (props: any) => {
    if (!props.pagination && props.checkboxSelectionVisibleOnly) {
      return new Error(
        'The `checkboxSelectionVisibleOnly` prop has no effect when the pagination is not enabled.',
      );
    }
    return null;
  }),
  ...
  filterMode: chainPropTypes(PropTypes.oneOf(['client', 'server']), (props: any) => {
    if (props.treeData && props.filterMode === 'server') {
      return new Error(
        'The `filterMode="server"` prop is not available when the `treeData` is enabled.',
      );
    }
    return null;
  }),
  ...
  hideFooterRowCount: chainPropTypes(PropTypes.bool, (props: any) => {
    if (props.pagination && props.hideFooterRowCount) {
      return new Error(
        'The `hideFooterRowCount` prop has no effect when the pagination is enabled.',
      );
    }
    return null;
  }),
